...
async def _index_data_content(task_instance, project_id: int, do_reset: int):
    ...
    try:
        ...
        while has_records:
            ...
            if not all(chunk.chunk_metadata for chunk in page_chunks):
                logger.error(
                    "Chunk metadata validation failed before vector indexing for project_id: %s, page_no: %s",
                    project.project_id,
                    page_no,
                )
                raise ValueError(
                    f"Chunk metadata missing before indexing for project_id: {project.project_id}"
                )

            logger.info(
                "Indexing %s chunks for project_id=%s with preserved metadata",
                len(page_chunks),
                project.project_id,
            )
