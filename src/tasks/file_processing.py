    ...
    try:
        ...
        for asset_id, file_id in project_files_ids.items():
            ...
            if len(file_content) > 0:
                logger.info(
                    "Loaded %s document parts for %s with sample metadata: %s",
                    len(file_content),
                    file_id,
                    file_content[0].metadata,
                )
            ...
            if not all(chunk.metadata for chunk in file_chunks):
                logger.error("Chunk metadata validation failed for file_id: %s", file_id)
                raise ValueError(f"Chunk metadata missing for file_id: {file_id}")

            logger.info(
                "Validated metadata for %s chunks from %s",
                len(file_chunks),
                file_id,
            )
